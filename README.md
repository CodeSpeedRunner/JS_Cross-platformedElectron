...
This is a workshop for learning how to build cross platform desktop apps with JavaScript, HTML, and CSS.
...
Now, let's prepare the app to be shipped to real users. This is called "packaging" an app. Nearly all of the magic is handled by a package.
...
3. :bowtie: Add emoji support.

4. Add file watching support. Make it so local files that are opened are watched for changes and the viewer will automatically update when a file has been changed. This makes it ideal for writing documents in your favorite text editor so you get a live preview. In "watch mode", the editor textarea should be hidden since the user will using their own editor.
